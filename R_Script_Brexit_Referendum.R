...
colvector = c('red', 'orange', 'yellow', 'green', 'cyan', 'blue') labvector = c('PC1', 'PC2', 'PC3') 
barplot(   pc_leave_loadings[,c(1:3)], yaxt = 'n', names.arg = labvector,   col = colvector,   ylim = c(-1,1),   border = 'white', 
ylab = 'loadings' 
) 
...
opar <- par() 
colvector = c('red', 'orange', 'yellow', 'green', 'cyan', 'blue') labvector = c('PC1', 'PC2', 'PC3') 
barplot(   pc_leave_loadings[,c(1:3)], 
beside = T, 
...
par(mfrow = c(2,2)) png(file = "percent") 

biplot(   pc_leave_clear,   scale = 0, 
col = c('grey40','orange') 
) 
...
opar <- par() par(mfrow = c(2,1)) 
plot(sil_hc_leave) png(file = "percent") 
plot(sil_hc_leave_complete) par(opar) 
...
predict_testNN = (pred_leave_nn_65$net.result * (max(test_leave$Outcome) - min(test_leave$Outcome))) + min(test_leave$Outcome) 
predict_testNN = (pred_leave_nn_64$net.result * (max(test_leave$Outcome) - min(test_leave$Outcome))) + min(test_leave$Outcome) 
predict_testNN = (pred_leave_nn_66$net.result * (max(test_leave$Outcome) - min(test_leave$Outcome))) + min(test_leave$Outcome) 
predict_testNN = (pred_leave_nn_67$net.result * (max(test_leave$Outcome) - min(test_leave$Outcome))) + min(test_leave$Outcome) 
predict_testNN = (pred_leave_nn_68$net.result * (max(test_leave$Outcome) - min(test_leave$Outcome))) + min(test_leave$Outcome) 
predict_testNN = (pred_leave_nn_69$net.result * (max(test_leave$Outcome) - min(test_leave$Outcome))) + min(test_leave$Outcome) 
...
1-mae(pred_leave_nn_64$net.result * (max(test_leave$Outcome) - min(test_leave$Outcome))) + min(test_leave$Outcome) 
1-mae(pred_concrete_nn_65$net.result * (max(test_concrete_minmax$Outcome) - min(test_concrete_minmax$Outcome))) + min(test_concrete_minmax$Outcome) 
1-mae(pred_concrete_nn_66$net.result * (max(test_concrete_minmax$Outcome) - min(test_concrete_minmax$Outcome))) + min(test_concrete_minmax$Outcome) 
1-mae(pred_concrete_nn_67$net.result * (max(test_concrete_minmax$Outcome) - min(test_concrete_minmax$Outcome))) + min(test_concrete_minmax$Outcome) 
1-mae(pred_concrete_nn_68$net.result * (max(test_concrete_minmax$Outcome) - min(test_concrete_minmax$Outcome))) + min(test_concrete_minmax$Outcome) 
1-mae(pred_concrete_nn_69$net.result * (max(test_concrete_minmax$Outcome) - min(test_concrete_minmax$Outcome))) + min(test_concrete_minmax$Outcome) 
...
cv_leave <- cv.tree(tree_leave, FUN=prune.misclass) cv_leave_table <- data.frame(size = cv_leave$size, error = cv_leave$dev ) 

plot(cv_leave,   xaxt = 'n', yaxt = 'n') 
...
leave_tree_predict <-  cbind( actual = test_leave$Outcome, 
...
tree_confmat <- confusionMatrix(data = leave_tree_predict$predicted, reference = leave_tree_predict$actual, positive = "Leave") 
rf_confmat <- confusionMatrix(data = leave_rf_predict$predicted, reference = leave_rf_predict$actual, positive = "Leave") 
...
leave_rf_predict <-  cbind(actual = test_leave$Outcome,predicted = predict(rf_leave, 
                                    test_leave[, -13], type = 'response'),
...
mymodel <- svm(Outcome ~ Mean_Income + Median_Income + Mean_Tax + Median_Tax + Percent.Turnout + C + La b + Green + UKIP + LD,
               data = leave_clear, kernel = "sigmoid") summary(mymodel) 
...
set.seed(2018) 
tmodel <- tune(svm,Outcome ~ Mean_Income + Median_Income + Mean_Tax + Median_Tax + Pct_Turnout + C + Lab + Green + UKIP + LD, 
               data = leave_clear, ranges = list(epsilon = seq(0,1,0.1), cost = 2^(2:5))) 
...
head(training_leave$Outcome) confusionMatrix(p1, training_leave$Outcome) 
p2 <- predict(rf_leave, test_leave) confusionMatrix(p2, test_leave$Outcome) 
